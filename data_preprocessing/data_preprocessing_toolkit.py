...
class DataPreprocessingToolkit(object):
    ...
    @staticmethod
    def filter_out_company_clients(df: pd.DataFrame):
        """
    ...
    @staticmethod
    def filter_out_long_stays(df: pd.DataFrame):
        """
    ...
    @staticmethod
    def filter_out_low_prices(df: pd.DataFrame):
        """
    ...
    @staticmethod
    def add_length_of_stay(df: pd.DataFrame):
        """
    ...
    @staticmethod
    def add_book_to_arrival(df: pd.DataFrame):
        """
    ...
    def add_weekend_stay(df):
        ...
        :rtype: pd.DataFrame
        """
        def get_day_names(row):
            date_range = pd.date_range(start=row['date_from'], end=row['date_to'])
            day_names = date_range.strftime('%A').tolist()
            if 'Friday' in day_names and 'Saturday' in day_names:
                return 'True'
            else:
                return 'False'

        df['weekend_stay'] = df.apply(get_day_names, axis=1)
    ...
    def aggregate_group_reservations(self, df):
        ...
        df = pd.concat([non_group_reservations, aggregated_group_reservations], axis=0)
    ...
    def map_night_prices_to_room_segment_buckets(self, df):
        ...
        :rtype: pd.DataFrame
        """
        # groupby room_group_id, take the mean of night_price and then map the mean to room_segment
